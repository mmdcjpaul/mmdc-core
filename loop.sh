...
build_prompt() {
  local id="$1" attempt="$2" verify_log="$3" spec title
...
EOF
    ...
    cat <<EOF
Repair ticket $id. Attempt $attempt of $MAX_ATTEMPTS.$([ "$attempt" -eq "$MAX_ATTEMPTS" ] && printf ' This is the final attempt.')
...
write_blocked() {
  local id="$1" title="$2" verify_log="$3"
  if [ ! -f "$BLOCKED" ]; then
...
while :; do
  ...
  log "$ticket — $title (model: $MODEL, reasoning: ${REASONING_EFFORT:-default})"
  ...
  while [ "$attempt" -le "$MAX_ATTEMPTS" ]; do
    ...
    build_prompt "$ticket" "$attempt" "$verify_log" \
  ...
  if [ "$passed" -ne 1 ]; then
    ...
    write_blocked "$ticket" "$title" "$verify_log"
    log "$ticket blocked after $MAX_ATTEMPTS attempts. Stopping."
    exit 1
  fi
